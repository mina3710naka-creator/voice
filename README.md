...
常駐開始時に `whisper-server`（モデルをメモリに常駐させたまま複数回の認識をさばくモード）を
バックグラウンドで自動的に起動します。起動直後の最初の一回だけは通常速度（モデルの読み込み待ち）
になりますが、以降はモデルの再読み込みが発生しないため大幅に高速化されます。設定画面の
「エンジンの状態」で高速モードが有効かどうか確認できます。
